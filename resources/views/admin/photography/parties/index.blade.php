...
<div class="container py-5">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap mb-4">
        <h1 class="mb-2 mb-md-0">Parties & Concerts</h1>
        <div class="d-flex gap-2 flex-wrap">
            @if($parties->count())
            <a href="{{ route('admin.photography.parties.edit', $parties->first()) }}" class="btn btn-warning shadow-sm">
                <i class="bi bi-pencil-square"></i> Edit Party
            </a>
            @else
            <a href="{{ route('admin.photography.parties.create') }}" class="btn btn-primary shadow-sm">
                <i class="bi bi-plus-circle"></i> Add Party
            </a>
            @endif
        </div>
    </div>
    ...
    @if($parties->count())
    @php $party = $parties->first(); @endphp
    <div class="card mb-5 shadow border-0 rounded-4 overflow-hidden">
        ...
        <div class="card-header bg-gradient-primary text-white fw-bold d-flex justify-content-between align-items-center py-3 px-4 flex-wrap">
            ...
            <div class="d-none d-md-flex gap-2 flex-wrap">
                <a href="{{ route('admin.photography.parties.edit', $party) }}" class="btn btn-outline-light btn-sm">
                    <i class="bi bi-pencil-square"></i> Edit
                </a>
                <a href="{{ route('admin.photography.parties.gallery', $party) }}" class="btn btn-outline-info btn-sm">
                    <i class="bi bi-images"></i> Manage Gallery
                </a>
                <form action="{{ route('admin.photography.parties.destroy', $party) }}" method="POST" onsubmit="return confirm('Delete this party entry?');">
                    @csrf
                    @method('DELETE')
            ...
            <div class="dropdown d-md-none">
                <button class="btn btn-light btn-sm dropdown-toggle" type="button" id="partyActions" data-bs-toggle="dropdown" aria-expanded="false">
                    Actions
                </button>
                <ul class="dropdown-menu" aria-labelledby="partyActions">
                    <li><a class="dropdown-item" href="{{ route('admin.photography.parties.edit', $party) }}">Edit</a></li>
                    <li><a class="dropdown-item" href="{{ route('admin.photography.parties.gallery', $party) }}">Manage Gallery</a></li>
                    <li>
                        <form action="{{ route('admin.photography.parties.destroy', $party) }}" method="POST" onsubmit="return confirm('Delete this party entry?');">
                            @csrf
                            @method('DELETE')
        ...
        <div class="row g-0 border-bottom flex-column flex-md-row">
            <div class="col-md-6 position-relative mb-3 mb-md-0">
                <img src="{{ $party->hero_image ? asset('storage/'.$party->hero_image) : 'https://via.placeholder.com/600x400' }}"
            ...
            <div class="col-md-6 p-4 bg-light">
                ...
                <img src="{{ asset('storage/'.$party->hero_right_image) }}" class="img-fluid rounded shadow-sm mb-3 hero-right-image">
                @else
        ...
        <div class="card-body py-3 px-4">
            ...
            <p class="text-secondary">{{ $party->description ?? $party->content ?? '-' }}</p>
        </div>

        <!-- GALLERY -->
        @if(!empty($party->gallery) && count($party->gallery) > 0)
        <div class="card-footer py-4 px-4 bg-white">
            <h5 class="fw-bold mb-3">Gallery</h5>
            <div class="row g-3">
                @foreach($party->gallery as $image)
    ...
    @else
    <p class="text-muted text-center fs-5 mt-5">No party entries found. Add one using the button above.</p>
    @endif
